...
def _base_categories(requested_scope: set[str], file_path: str, static_types: set[str]) -> list[str]:
    categories: list[str] = []
    ...
    if "security" in requested_scope and ("auth-change" in static_types or any(token in path for token in ("auth", "token", "secret", "session"))):
...
def plan_hotspot_tasks(
    ...
    *,
    ...
    for file in request.files:
        ...
        categories = _base_categories(requested_scope, file.path, static_types)
        if not categories:
            categories = sorted(requested_scope)[:1]
